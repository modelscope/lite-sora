...
class SDTextEncoder(torch.nn.Module):
    ...
    def forward(self, input_ids, clip_skip=1, pooled_output_id=None):
        ...
        if pooled_output_id is not None:
            pooled_output = embeds[
                torch.arange(embeds.shape[0]),
                (input_ids.to(dtype=torch.int, device=embeds.device) == pooled_output_id).int().argmax(dim=-1),
            ]
        else:
            pooled_output = None
