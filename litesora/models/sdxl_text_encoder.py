...
class SDXLTextEncoder(torch.nn.Module):
    ...
    def from_diffusers(file_path=None, state_dict=None):
        ...
        state_dict = SDXLTextEncoderStateDictConverter().from_diffusers(state_dict)
    ...
    def from_civitai(file_path=None, state_dict=None):
        if state_dict is None:
            state_dict = load_state_dict(file_path)
        state_dict = SDXLTextEncoderStateDictConverter().from_civitai(state_dict)
        model = SDXLTextEncoder()
        model.load_state_dict(state_dict)
        return model
...
class SDXLTextEncoder2(torch.nn.Module):
    ...
    def from_diffusers(file_path=None, state_dict=None):
        if state_dict is None:
            state_dict = load_state_dict(file_path)
        state_dict = SDXLTextEncoder2StateDictConverter().from_diffusers(state_dict)
    ...
    def from_civitai(file_path=None, state_dict=None):
        if state_dict is None:
            state_dict = load_state_dict(file_path)
        state_dict = SDXLTextEncoder2StateDictConverter().from_civitai(state_dict)
        model = SDXLTextEncoder2()
        model.load_state_dict(state_dict)
        return model
